...
class Laser(ABC):
    ...
    def get_laser_reading(self, root: KinematicStructureEntity) -> LaserReading:
        """
        :param root: The entity in the world the beams originate from.
...
class SimulatedLaser(Laser):
    ...
    def get_laser_reading(self, root: KinematicStructureEntity) -> LaserReading:
        world_T_laser = root.global_transform.to_np()
        ...
        points, index_ray, _ = root._world.ray_tracer.ray_test(
            world_P_laser,
        ...
        return LaserReading(
            direction=self.pattern.beam_directions_in_frame(root),
